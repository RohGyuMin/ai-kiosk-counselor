...
| **AI 상담** | 브라우저 음성 인식(STT) → Claude 답변 생성 → 음성 출력(TTS) + 동기 안내 이미지 |
...
### ① 음성은 브라우저에서, 답변 생성만 서버에서
- **STT/TTS = 브라우저 내장 Web Speech API.** 별도 음성 서버·키가 필요 없고 지연이
  낮다. 키오스크 PC의 브라우저만으로 음성 입출력이 완결된다.
- **답변 생성 = 서버 라우트(`/api/chat`)에서만 Claude 호출.** API 키를 클라이언트에
  절대 노출하지 않는다 → 이 저장소가 **Public이어도 안전**하다.
- 미지원 브라우저는 자동으로 **텍스트 모드로 강등**된다.
