...
// DB 호출은 server-only 파일 clinic-server.ts로 분리되어 있다.
...
// buildSystemPrompt / getEffectiveClinic / getCurrentEditable 는 ./clinic-server.ts 참조
// (DB 접근이 있어 server-only — 클라이언트 번들로 끌려가지 않도록 분리)
